...
rotational_distortion::rotational_distortion()
{
}
//构造函数
void rotational_distortion::get_rotational_distortion(QImage img_src,Point2f centre,float r,float ang)
{
...
//映射函数，求出目标图中的点对应的原图中的点
rotational_distortion::Point2f rotational_distortion::affine(Point2f point_dst)
{
    Point2f point_temp;
    //相对于旋转中心点的坐标
    ...
    float angle_b = 3.1415926 * angle/180;

    Point2f point_src;
    //根据旋转反变换公式求出原图中对应点
    point_src.x = r*cosf(angle_a + angle_b*(radius - r)/radius) + center.x;
    point_src.y = r*sinf(angle_a + angle_b*(radius - r)/radius) + center.y;
...
//最近邻插值
QRgb rotational_distortion::insert_closest(Point2f point_src)
{
    Point2f point_temp;
    //四舍五入求最近邻点
...
//双线性插值
QRgb rotational_distortion::insert_bilinear(Point2f point_src)
{
    point_src.x = point_src.x >= image_src.width()? image_src.width() - 1:point_src.x;
    point_src.y = point_src.y >= image_src.height()? image_src.height() - 1:point_src.y;
    //求出该点所处网格的四个顶点
...
//三次插值核函数
float rotational_distortion::S(float x)
{
    float y = fabs(x);
    float S;
    if(y <= 1)
...
//获取像素点的值
float rotational_distortion::get_value(int i,int j,int flag,QImage image)
{
...
//双三次插值
QRgb rotational_distortion::insert_bicubic(Point2f point_src)
{
...
//判断是否在扭曲圆中
bool rotational_distortion::in_circle(Point2f point_dst)
{
...
//最近邻插值变换
void rotational_distortion::transform_closest()
{
    Point2f point_dst;
    Point2f point_src;
...
//双线性插值变换
void rotational_distortion::transform_bilinear()
{
    Point2f point_dst;
    Point2f point_src;
    QPoint qpoint;
...
//双三次插值变换
void rotational_distortion::transform_bicubic()
{
    Point2f point_dst;
    Point2f point_src;
    QPoint qpoint;
    for(int i=0;i < image_dst.width();i++)
    {
        for(int j=0;j < image_dst.height();j++)
        {
            point_dst.x = i;
            point_dst.y = j;
            if(in_circle(point_dst))
